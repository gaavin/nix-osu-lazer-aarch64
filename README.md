...
```bash
nix run github:gaavin/nix-osu-lazer-aarch64
```
...
```nix
{
  inputs = {
    ...
    nix-osu-lazer-aarch64.url = "github:gaavin/nix-osu-lazer-aarch64";
    nix-osu-lazer-aarch64.inputs.nixpkgs.follows = "nixpkgs";
  };

  outputs = { self, nixpkgs, home-manager, nix-osu-lazer-aarch64, ... }:
    {
...
```nix
{
  programs.nix-osu-lazer-aarch64.enable = true;
}
```
...
```bash
nix flake update nix-osu-lazer-aarch64
...
| Issue | Solution |
|-------|----------|
| Window never appears | Wayland is the default (`SDL_VIDEODRIVER=wayland`). Set `programs.nix-osu-lazer-aarch64.nativeWayland = false;` for X11/XWayland. |
| No audio | PipeWire/ALSA must be running. This package sets the same audio runtime deps as nixpkgs `osu-lazer`. |
| Self-updater / "update available" | The wrapper sets `OSU_EXTERNAL_UPDATE_PROVIDER=1`. Bump `version` + hashes in `pkgs/nix-osu-lazer-aarch64/` instead. |
...
```nix
home.packages = [
  inputs.nix-osu-lazer-aarch64.packages.${pkgs.stdenv.hostPlatform.system}.nix-osu-lazer-aarch64
];
```

```bash
nix build github:gaavin/nix-osu-lazer-aarch64
```
